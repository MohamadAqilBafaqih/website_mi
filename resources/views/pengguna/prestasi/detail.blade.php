                    ...
                    <div class="prestasi-img-wrapper">
                        @if($prestasi->foto)
                            <img src="{{ asset('uploads/prestasisiswa/' . $prestasi->foto) }}" 
                                class="img-fluid rounded-start prestasi-img" 
                    ...
                    <div class="card-body p-4">
                        ...
                        <ul class="list-unstyled mb-3">
                            <li><i class="fas fa-user me-2 text-success"></i> 
                                <strong>Nama Siswa:</strong> {{ $prestasi->nama_siswa }}</li>
                            <li><i class="fas fa-school me-2 text-success"></i> 
                                <strong>Kelas:</strong> {{ $prestasi->kelas }}</li>
                            <li><i class="fas fa-trophy me-2 text-success"></i> 
                                <strong>Tingkat:</strong> {{ $prestasi->tingkat ?? '-' }}</li>
                            <li><i class="fas fa-calendar-alt me-2 text-success"></i> 
                                <strong>Tanggal:</strong> {{ $prestasi->tanggal ? \Carbon\Carbon::parse($prestasi->tanggal)->format('d M Y') : '-' }}</li>
                            <li><i class="fas fa-building me-2 text-success"></i> 
                                <strong>Penyelenggara:</strong> {{ $prestasi->penyelenggara ?? '-' }}</li>
                        </ul>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0">
                <div class="modal-body p-0">
                    <img src="{{ asset('uploads/prestasisiswa/' . $prestasi->foto) }}" class="img-fluid w-100" alt="{{ $prestasi->nama_prestasi }}">
                </div>
...
/* Gambar */
.prestasi-img-wrapper { overflow: hidden; height: 100%; }
.prestasi-img { width: 100%; height: 100%; object-fit: cover; cursor: pointer; transition: transform 0.4s ease; }
.prestasi-img:hover { transform: scale(1.05); }

/* List */
ul.list-unstyled li { padding: 6px 0; border-bottom: 1px solid #f1f1f1; }
ul.list-unstyled li:last-child { border-bottom: none; }
</style>
